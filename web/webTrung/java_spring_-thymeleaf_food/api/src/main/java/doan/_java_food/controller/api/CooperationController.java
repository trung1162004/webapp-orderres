...
public class CooperationController {
    ...
    public BaseResponse<?> findAll(
            ...
            @RequestParam(name = "service_id", required = false, defaultValue = "") String service_id,
        ...
        try {
            ...
            var response = service.findAndCount(number_page, Integer.parseInt(page_size), status, service_id, user_id);
    ...
    @PostMapping("store")
    public BaseResponse<?> save( @RequestHeader(HttpHeaders.AUTHORIZATION) String author,@RequestBody CooperationRequestDto data) {
        try {
            String access_token = author.replace("Bearer ", "");
            var user = userApiService.findByAccessToken(access_token);
            if(user == null) {
    ...
    public BaseResponse<?> update(@RequestHeader(HttpHeaders.AUTHORIZATION) String author, @PathVariable Long id, @RequestBody CooperationRequestDto data) {
        try {
            String access_token = author.replace("Bearer ", "");
            var user = userApiService.findByAccessToken(access_token);
            if(user == null) {
